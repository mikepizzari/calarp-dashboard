...
<main>
  ...
  <div class="card">
    ...
    <div class="filter-row">
      ...
      <button class="filter-btn" data-pf="P1">P1</button>
      <button class="filter-btn" data-pf="P2">P2</button>
      <button class="filter-btn" data-pf="P3">P3</button>
      <div class="filter-sep"></div>
...
function submitLogForm(key) {{
  ...
  fetch(APPS_SCRIPT_URL+'?'+qs.toString())
    .then(r=>r.text())
    .then(txt=>{{
      if(txt&&txt.startsWith('error')){{
        if(msg){{msg.textContent=txt;msg.className='log-msg err';}}
...
// ── Table ─────────────────────────────────────────────────────────────────────
let terrFilter='all', tierFilter='all', priorityFilter='all', stateFilter='', search='', page=1, sortCol='t', sortAsc=true, expandedKey=null, logFormKey=null, editFormKey=null;
const PAGE=20;
...
function getFiltered(){{
  return LEADS.filter(l=>{{
    ...
    const fp = priorityFilter==='all' ? true : _followupFlag(l,CRM[String(l.k)]||null)===priorityFilter;
    const fs = stateFilter ? l.st===stateFilter : true;
    const fq = l.n.toLowerCase().includes(search);
    return ft&&fi&&fp&&fs&&fq;
  }}).sort((a,b)=>{{
    if(sortCol==='rv'){{
      const o={{overdue:0,soon:1,ok:2,unknown:3}};
      const va=(o[a.rv]??3), vb=(o[b.rv]??3);
...
document.querySelectorAll('.filter-btn[data-pf]').forEach(btn=>{{
  btn.addEventListener('click',()=>{{
    document.querySelectorAll('.filter-btn[data-pf]').forEach(b=>b.classList.remove('active'));
    btn.classList.add('active'); priorityFilter=btn.dataset.pf; page=1; render();
  }});
}});
